...
| In Progress | Queue setup | `php artisan production:check` now requires `QUEUE_WORKER_ENABLED=true` and documents `QUEUE_WORKER_COMMAND`. Remaining work: configure a Coolify worker process/service running `php artisan queue:work --sleep=3 --tries=3 --timeout=120` and monitor failed jobs. |
