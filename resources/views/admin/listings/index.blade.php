                ...
                <div class="p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead><tr><th class="px-3 py-2 text-left"></th><th class="px-3 py-2 text-left">Title</th><th class="px-3 py-2 text-left">Status</th><th class="px-3 py-2 text-left">City</th><th class="px-3 py-2 text-left">Actions</th></tr></thead>
                            <tbody class="divide-y divide-gray-100">@foreach ($listings as $listing)<tr><td class="px-3 py-2"><input class="row_cb rounded border-gray-300" type="checkbox" name="ids[]" value="{{ $listing->slug }}"></td><td class="px-3 py-2">{{ $listing->title }}</td><td class="px-3 py-2">{{ ucfirst($listing->status) }}</td><td class="px-3 py-2">{{ $listing->city }}</td><td class="px-3 py-2"><div class="flex gap-3"><a href="{{ route('admin.listings.edit', $listing) }}" class="text-indigo-600">Edit</a><form method="post" action="{{ route('admin.listings.destroy', $listing) }}">@csrf @method('DELETE')<button class="text-red-600" type="submit" onclick="return confirm('Delete this listing?');">Delete</button></form></div></td></tr>@endforeach</tbody>
                        </table>
                    </div>
                    </form>
